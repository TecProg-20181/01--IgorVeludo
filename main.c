...
int max(int a, int b) {
    if (a > b)
        return a;//nome ruim de variaveis
    return b;
}
...
Image rotacionar90direita(Image img) {
    ...
    return rotacionada;
}//fim do metodo rotacionar90direita
...
}//fim do metodo inverter cores

Image cortar_imagem(Image img, int x, int y, int wCortado, int hCortado) {
    Image cortada;

    cortada.w = wCortado;
    cortada.h = hCortado;

    for(int i = 0; i < hCortado; ++i) {
        for(int j = 0; j < wCortado; ++j) {
